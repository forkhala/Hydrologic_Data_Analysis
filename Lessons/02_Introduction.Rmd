...
For results of experiments, variables to be visualized could be the independent variable (explanatory variable) and the dependent variable (response variable) that are being investigated. Using a graph, one may want to convey the relationship between the two variables (e.g. linear, polynomial, logarithm etc) for continuous independent variables and the difference between groups for categorical/discrete variables
<add notes here>
...
Scatterplot would be the best to explain the relationship between two continuous vars.
<add notes here> 
...
- Try to avoid pie chart, as we are not good at comparing angles. Barplot/histogram could be an alternative.
...
```{r}
EnoDischarge <- mutate(EnoDischarge, Discharge.m3 = Discharge/35.3147)
...
```{r}
EnoDischarge <- mutate(EnoDischarge, rmean.30d = rollmean(EnoDischarge$Discharge.m3, 
                                                          30, fill = NA))
...
5. Edit axes
```{r}
EnoPlot2 <- ggplot(EnoDischarge, aes(Date))+
  theme_classic()
print(EnoPlot2)
```
