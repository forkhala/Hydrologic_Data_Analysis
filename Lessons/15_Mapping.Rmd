...
* **Point** <one longitude value and one latitude value; fit tidyverse dataframe well>
* **Line** <two long values and two lat values; do not fit tidyverse dataframe very well>
* **Polygon** <at least three points that do not lie in a line; may be represented by the central point of the polygon>
...
LAGOScombined <- 
  ...
  #if not specifying the "by=" arugment, R will automatically find the column(s) by which dfs will be joined
...
secchi.summary <- LAGOScombined %>%
  ...
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha), # LAGOS datset has same area, depth, lat, long for each lake. We take means because we reduce multiple rows of each lakeid into one row.
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>%
  drop_na()
...
Secchiplot <- ggplot() +
  geom_sf(data = states.subset, fill = "white") + # plots built sequentially
...
```{r}
state.me <- filter(states, ID %in% "maine")
secchi.me <- LAGOScombined %>%
  filter(state == "ME") %>%
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
            # LAGOS datset has same area, depth, lat, long for each lake. We take means because we reduce multiple rows of each lakeid into one row.
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>%
  drop_na()
...
```{r}
secchi.spatial.me <- st_as_sf(secchi.me, coords = c("long", "lat"), crs = 4326)

secchiplot.me.1 <- ggplot() +
  geom_sf(data = state.me, fill = "white")+
  geom_sf(data = secchi.spatial.me, aes(color = secchi.mean, size = area), 
          alpha = 0.7)+
  scale_color_viridis_c(option = "inferno") +
  labs(color = "Average Secchi Depth (m)", size = "Area (ha)") +
  theme(legend.position = "top") +
  guides(color = F)
print(secchiplot.me.1)
...
```{r}
plot_grid(secchiplot.me.1, secchiplot.me.2, ncol = 2, nrow = 1)
```
...
anim_save('Secchi.Animation1.gif', animate(Secchi.Animation, height = 400, wid = 600,
