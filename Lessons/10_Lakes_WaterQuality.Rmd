...
> Variable 1: state( state)

> Variable 2: Secchi depth (epi_nutr)

> Variable 3: maxdepth (lakes_limno)

> Variable 4 doc (epi_nutr)

> Variable 5: chlorophyll a (epi_nutr)
...
LAGOStrophic <- 
  ...
  left_join(., LAGOSlimno, by = "lagoslakeid") %>%
  select(lagoslakeid, sampledate, tn, tp, state, secchi, maxdepth, doc, chla,
         gnis_name, lake_area_ha, state_name) %>%
...
cor.m <- cor(LAGOStrophic[,c(3:4, 6:9)])
corplot <- corrplot(cor.m, method = "ellipse", type = "upper")
```
...
model.tn <- glm(tn ~ state + secchi + maxdepth + doc + chla, data = LAGOStrophic)
summary(model.tn)
anova(model.tn)

model.tp <- glm(tp ~ state + secchi + maxdepth + doc + chla, data = LAGOStrophic)
summary(model.tn)
anova(model.tp)
