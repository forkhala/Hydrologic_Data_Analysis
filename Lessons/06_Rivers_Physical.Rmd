...
> Timing of increase of discharge in spring tends to be earlier due to warmer temperature.
Higher evaporation due to inceased temperature might reduce the flow rate.
Rise in sea level -> intrusion of salt water into freshwater system -> change the salinity
...
> Stationary behavior: characteristics of changes within a year stay relatively constant over years
...
EnoRecurrence <- 
  ...
  mutate(Rank = rank(-PeakDischarge), # Note -PeakDischarfe
...
# First parameter is intercetp; second parameter is slope
...
> With a longer periods (full range of the data), the linear model predicts a higher peak discharge given the same R.I. than the model constructed from data before 1993.
...
> Try to use as many avilable data points as possible to construct a predictory model.
Alternatively, we cna use moving window (i.e. the most recent data) to construct models for each year.
...
# add more code here:
# Using the first 30 years

SaladoDischarge <- 
  SaladoDischarge %>%
  mutate(Year = year(Date))
range(SaladoDischarge$Year)

SaladoRecurrence.old <- 
  SaladoDischarge %>%
  filter(Year < 1990) %>%
  group_by(Year) %>%
  summarise(PeakDischarge = max(Discharge)) %>% 
...
SaladoRecurrencePlot <- 
  ggplot(SaladoRecurrence.old, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() #+
  #scale_x_log10() 
print(SaladoRecurrencePlot)

Salado.RImodel.old <- lm(data = SaladoRecurrence.old, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.old)

Salado.RImodel.old$coefficients[1] + Salado.RImodel.old$coefficients[2] * log(100)
Salado.RImodel.old$coefficients[1] + Salado.RImodel.old$coefficients[2] * log(500)

# Using last 30 years data

SaladoRecurrence.new <- 
  SaladoDischarge %>%
  filter(Year > 1989) %>%
  group_by(Year) %>%
  summarise(PeakDischarge = max(Discharge)) %>% 
...
SaladoRecurrencePlot.new <- 
  ggplot(SaladoRecurrence.old, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  geom_point(data = SaladoRecurrence.new, color = "#02818a",
             aes(x = RecurrenceInterval, y = PeakDischarge))
print(SaladoRecurrencePlot.new)

Salado.RImodel.new <- lm(data = SaladoRecurrence.new, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.new)

Salado.RImodel.old$coefficients[1] + Salado.RImodel.old$coefficients[2] * log(100)
Salado.RImodel.old$coefficients[1] + Salado.RImodel.old$coefficients[2] * log(500)

Salado.RImodel.new$coefficients[1] + Salado.RImodel.new$coefficients[2] * log(100)
Salado.RImodel.new$coefficients[1] + Salado.RImodel.new$coefficients[2] * log(500)
