...
> Dissolved O2, nutrients - N, P, trace element , microbes, ordor/color, invasive species, heavy metals (contaminants)
...
# Run the command once on one computer to download the database; unless updates have been made for data
...
# Exploring the data types that are available (metadata)
...
> Factors that influence transparency: algae, sediment, dissolved organic matter
...
LAGOStrophic <- 
  ...
  drop_na(chla:secchi)
  # Remove rows with NA in ANY of the three coloumns Chla, tp, sechi
...
print(chlbymonth)
# Data within a month are spred over the width of that month category
...
ggplot(LAGOStrophic, 
  ...
  scale_color_viridis_c(option = "magma")
print(secchibymonth)
# Sacchi depth is generally opposite to chlorophyll conc.
...
> There is concurrent increase in nutrient and chla during summer in comparison to winter. Secchi depth observations are more concentrated near the low values during summer, which is consistent with the observations on nutrient and chla. Also there is noticeably more sampling effort during summer than winter
...
# Note the thre indices do not yield the same conclusion on trophic state, and there is no consistant difference among the three indices (e.g. one is always larger than another one). However, chla is used to determine eutrophic state, since it directly results from eutrophic state.
...
> Data points are concentrated below the 1:1 line, suggesting that chlorophyll predicts higher trophic status than Secchi depth and P. Also, when P is below about 30, P suggests oligotrophic whereas chl suggests mesotrophic to eutrophic
...
> Violin: distribution of data points along the whole range; where most data points are concentrated

> Bar: clear comparison of the counts of different categories (e.g. different trophic status levels)
...
```{r}
write.csv(LAGOStrophic, file = "./Data/LAGOS.csv", row.names = F)
