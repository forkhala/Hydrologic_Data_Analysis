...
LAGOSdata <- lagosne_load(fpath = "D:/William/Duke/Study/EOS 722/Hydrologic_Data_Analysis/Data/Raw/data_1.087.3.rds") 
# Load LAGOSNE data from local LAGOS temporary files
load(file = "./Data/Raw/.rda")
```
...
> aggregation: merge multiple data into one category, reducing rows of dataset. Methods used for aggregation depend on question, e.g. mean, range.

> subsampling: Applied when aggregation may change the variability of data unintentionally, e.g. unequal time frequency during two periods (daily vs weekly) and aggregating daily data into weekly may mask variability & trends

> interpolation: predicting gaps based on the nearest available data
...
* **modified Mann-Kendall**: no seasonality, non-parametric, accounts for temporal autocorrelation, missing data allowed. Function: `mmky` and `mmkh` (package: modifiedmk). (More conservative than original M-K function if autocorrelation is potentially present)
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution (constant gap betwee each observation). Function: `smk.test` (package: trend)
...
table(diff(Mendotadata$sampledate))
...
* **Piecewise constant**: <Nearest neighboring data interpolation - use the nearest available data to replace gaps>
* **Linear**: <Connect availble data by straight segments and predict the missing  point using the linear relationship>
* **Spline**: <Nonlinear relationship>
* Some other advacned techniques: Bayesian inference using other concurrent available variables during the time period in which one variable is missing
* If the dataset has a very long time span, we can use values from other years on the same day to interpolate missing values.
...
# Note in results from interpolation function, x has an origin on 1970 Jan 01
linearinterpolation$x <- as.Date(linearinterpolation$x, origin = "1970-01-01")
names(linearinterpolation) <- c("Date", "TP")

# WARNING: here n = 303 leads to 30 days a month, which will cause mismatch after converting to time-series (ts). n set to number of days and take average by group to make ts consistent with results from interpolation
# Number of days: 
difftime(max(Mendotadata$sampledate), min(Mendotadata$sampledate), "days")
linearinterpolation.update <- 
  as.data.frame(approx(Mendotadata, n = 9162, method = "linear"))
linearinterpolation.update$x <- 
  as.Date(linearinterpolation.update$x, origin = "1970-01-01")
names(linearinterpolation.update) <- c("Date", "TP")
...
ggplot(Mendotadata, aes(x = sampledate, y = tp)) +
  geom_point() +
  geom_line() +
  geom_point(data = linearinterpolation.update, aes(x = Date, y = TP), color = "#c13d75ff") 
...
# This will give a ts with 302 observations, which is 1 less than 303 obs

linearinterpolation.update <- linearinterpolation.update %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>%
  summarise(TP = mean(TP))

Mendotatimeseries.monthly <- ts(linearinterpolation.update$TP, frequency = 12, 
                        start = c(1988, 7), end = c(2013, 8))

# Run SMK test
# Mendotatrend <- smk.test(Mendotatimeseries)
Mendotatrend.monthly <- smk.test(Mendotatimeseries.monthly)

# Inspect results
Mendotatrend.monthly
# the sign of z-scores indicates the direction of trend; p-value for significance 
# S is the difference between two points; the absolute value of S reflect the magnitude of the trend
summary(Mendotatrend.monthly)
```

What would we conclude based on these findings? Describe as you would to an educated but non-specialist audience.

> There is no overall trend across the whole time span of dataset. Yet, October and November have a negative trend, while all other months show no changes. (NOT TRUE)
...
# Aggregate data into month average
...
```{r}
# Test whether or not dataset is stationary; and the H0 is not stationary
adf.test(ClearCreek_ts, alternative = "stationary")
# We conclude that data are stationary
```
...
# ARIMA(1,0,1)(2,1,0)[12] is selected, suggesting that the best-fit model has the combination of 1st order AR and 1st order MA models
