...
```
> Seasonal patterns vs cyclic pattern: the intervals of cyclic patterns could be larger than season, e.g. a few years, El Nino.
...
> No. It does not reflect the seasonality
...
> Predicting annual mean discharge with years.
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic (i.e. not necessarily be unidirectional).
...
```{r}
# if there is N/A, simply dropping the N/A will lead to an unequal spacing data; we need to interpolate the missing values.

# Check if there is gap in date, i.e. missing data on particular date(s)
...
# Trend is not very regular and changes constantly throughout the whole time span
...
# Note the trend is always positive, not because the difference compared to the initial vale is always positive, but instead because of the mathmatical calculation
