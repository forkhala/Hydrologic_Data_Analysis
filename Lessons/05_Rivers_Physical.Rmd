...
- Discharge m^3/s (velocity multiplied by cross-section area) vs velocity m/s
...
DvsGHplot <-
  ggplot(EnoDischarge.ratingcurve, aes(y = Discharge, x = Gage.Height)) +
...
> Each row represents one variable/statistic measured at the same location
...
EnoDischarge <- readNWISdv(siteNumbers = "02096500",
                     ...
                     startDate = "2009-09-01", # Delete date to get the earliest one
                     endDate = "2019-08-31") # Delete date to get the latest one
...
NehalemDischarge <- readNWISdv(siteNumbers = "14299800",
                     ...
                     startDate = "2009-09-01",
                     endDate = "2019-08-31")
# Note the variable is average discharge (00060_00003)
```
...
> Eno river has the highest among all five rivers except verde river, but all them are at the magnitude of around 10^4
...
> Bitterrot river, sauk river, nehalem river have seasonal cycle, and all of three have th highest discharge during spring. So the peaks are probably due to spring runnoff.
> The seasonal cycle in Sauk river particularly was disrupted during the last five years, probably due to warming winters
...
> Create a column specifying lake name and combine by rows (rbind())
...
> By looking at discharge vs date, we can determine if the river is driven by rainfall or driven by snow-melt. Also, disruption of seasonal cycle can also be visualized with graphs of discharge vs date.
